...
def detect_blinks_stream(
    ...
    closed_last_ts: Optional[float] = None

    # Create FaceMesh with a safe fallback if refine_landmarks isn't available
    mp_face_mesh = mp.solutions.face_mesh
    try:
        face_mesh = mp_face_mesh.FaceMesh(
            static_image_mode=False,
        ...
        logging.warning("MediaPipe FaceMesh refine_landmarks unavailable, falling back to refine_landmarks=False")
        face_mesh = mp_face_mesh.FaceMesh(
            static_image_mode=False,
    ...
    try:
        for frame, ts in frame_gen:
            ...
            if ear < ear_thresh:
                ...
                if closed_streak >= min_closed_frames and closed_start_ts is not None and closed_last_ts is not None:
                    center_ts = closed_start_ts + (closed_last_ts - closed_start_ts) / 2.0
                    blink_times.append(center_ts)
                    blink_durations.append(max(0.0, closed_last_ts - closed_start_ts))
                closed_streak = 0
                closed_start_ts = None
                closed_last_ts = None
    finally:
...
def analyze_blinks(video_path: str,
    ...
    logging.info("Blink analysis finished: blinks=%d, score=%.2f, class=%s", metrics.get("blink_count", 0), score, classification)

    return BlinkResult(suspicious_spans=out_spans, details=details, metric="blink_no_gap", score=score, classification=classification)
